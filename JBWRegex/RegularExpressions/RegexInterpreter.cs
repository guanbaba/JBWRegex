...
namespace JBWRegex.RegularExpressions
{
    ...
    internal sealed class RegexInterpreter : RegexRunner {
        ...
        private Word Forwardwordnext()
        {
            Word word = (runrtl ? runwordlist[--runtextpos] : runwordlist[runtextpos++]);
            return word;
        }
        ...
        private bool Wordsmatch(String str)
        {
            int c;
            int pos;
        ...
        private bool RefWordmatch(int index, int len)
        {
            int c;
            int pos;
            int cmpos;
        ...
        private Word WordAt(int j)
        {
            return runwordlist[j];
        }
        ...
        protected override void Go2()
        {
            ...
            for (; ; )
            {
                ...
                switch (Operator())
                {
                    ...
                    case RegexCode.Bol:  //一位操作码
                        if (Leftchars() > 0 && WordAt(Textpos() - 1).word != "\n")   //* 左边存在字符且不等于换行符
                            break;
                        Advance();
                        continue;

                    case RegexCode.Eol:     //一位操作码
                        if (Rightchars() > 0 && WordAt(Textpos()).word != "\n")      //* 右边存在字符且不等于换行符
                            break;
                        Advance();
                        continue;
                    ...
                    case RegexCode.EndZ:    //一位操作码
                        if (Rightchars() > 1 || Rightchars() == 1 && WordAt(Textpos()).word != "\n")
                            break;
                        Advance();
                        continue;
                    ...
                    case RegexCode.Ref:     //两位操作码
                        {
                            ...
                            if (IsMatched(capnum))
                            {
                                if (!RefWordmatch(MatchIndex(capnum), MatchLength(capnum))) //* 判断后向引用是否相等
                                    break;
                            }
                            else
                            {
