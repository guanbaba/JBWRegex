...
namespace JBWRegex.RegularExpressions {
...
#endif
    {
        ...
        */
        ...
        public bool IsMatch(List<Word> wordlist)
        {

            if (wordlist == null)
                throw new ArgumentNullException("input");

            return IsMatch(wordlist, UseOptionR() ? wordlist.Count : 0);
        }
        ...
        public bool IsMatch(List<Word> wordlist, int startat)
        {

            if (wordlist == null)
                throw new ArgumentNullException("input");

            return (null == Run(true, -1, wordlist, 0, wordlist.Count, startat));
        }
        ...
        public Match Match(List<Word> wordlist)
        {
        ...
        public Match Match(List<Word> wordList, int startat)
        {

            if (wordList == null)
                throw new ArgumentNullException("input");

            return Run(false, -1, wordList, 0, wordList.Count, startat);
        }
        ...
        internal Match Run(bool quick, int prevlen, List<Word> wordList, int beginning, int length, int startat)
        {
            Match match;
            RegexRunner runner = null;
            ...
            {
                // Do the scan starting at the requested position            
                match = runner.Scan(this, wordList, beginning, beginning + length, startat, prevlen, quick, internalMatchTimeout);
            }
            finally
            {
                // Release or fill the cache slot
