...
namespace JBWRegex.RegularExpressions
{
    ...
    public class Match : Group {
        ...
        internal Match(Regex regex, int capcount, List<Word> wordlist, int begpos, int len, int startpos)

            : base(wordlist, new int[2], 0, "0")
        {
            ...
            // No need for an exception here.  This is only called internally, so we'll use an Assert instead
            System.Diagnostics.Debug.Assert(!(_textbeg < 0 || _textstart < _textbeg || _textend < _textstart || _wordlist.Count < _textend),
                                            "The parameters are out of range.");
        ...
        internal virtual void Reset(Regex regex, List<Word> wordlist, int textbeg, int textend, int textstart)
        {
            _regex = regex;
            _wordlist = wordlist;
            _textbeg = textbeg;
        ...
        static internal Match Synchronized(Match inner) {
            ...
            for (int i = 0; i < numgroups; i++) {
                ...
                JBWRegex.RegularExpressions.Group.Synchronized(group);
            }
    ...
    internal class MatchSparse : Match {
        ...
        internal MatchSparse(Regex regex, Hashtable caps, int capcount,
#endif
                             List<Word> wordlist, int begpos, int len, int startpos)

            : base(regex, capcount, wordlist, begpos, len, startpos)
        {
